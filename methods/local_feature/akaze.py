...
def run(img_path, cfg):
    ...
    if cfg.method_dict['config_common']['keypoint'].lower() == 'akaze-def':
        feature = cv2.AKAZE_create()
    elif cfg.method_dict['config_common']['keypoint'].lower() == 'akaze-lowth':
        feature = cv2.AKAZE_create(threshold=-10000)
    else:
